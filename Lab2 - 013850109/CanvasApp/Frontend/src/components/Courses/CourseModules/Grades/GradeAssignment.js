...
export default class TakeAssignment extends Component {
    ...
    onSubmit(e) {
        ...
        axios.post('http://' + rooturl + ':3001/studentassignment/' + this.props.match.params.courseUid + '/' + this.props.match.params.assignmentUid + '/' + encodeURI(this.state.assignmentInfo.student_emailId), data)
            .then((response) => {
                console.log(response);
                if (response !== undefined)

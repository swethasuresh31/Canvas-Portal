...
class Account extends Component {
  ...
  async profileImage() {
    ...
    if (this.fileInput.current.files[0] !== undefined) {
      ...
      await axios.post('http://localhost:3001/account/img/' + encodeURI(this.state.emailId), data)
        .then((response) => {
  ...
  async componentWillMount() {
    ...
    console.log("finished mount of account")
    console.log("Store result: " + JSON.stringify(this.props))
    const result = this.props.profileStateStore.result;
    if (result) {
      var keyData = Object.keys(result.data);
  ...
  render() {
    if (localStorage.userToken) {
      if(!this.props.profileStateStore.result) return null;
